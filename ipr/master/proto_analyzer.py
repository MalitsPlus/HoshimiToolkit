...
master_name_ptn = r"public sealed class (?P<name>\w+)"
master_properties_ptn = r"public const int (?P<fieldName>\w+) = (?P<value>\d+);[\s\S]*?private (readonly )?(?P<type>[\w<>]+) (?P<name>\w+);"
...
{pairs}}
"""
master_pairs = "    [ProtoMember({value})] public {type} {name} { get; set; }\n"
master_text = """[ProtoContract]\npublic class {name} {
{pairs}}
"""
...
def gen_master(name: str, properties: list) -> str:
    pairs = ""
    for ppt in properties:
        pairs += master_pairs.replace("{value}", ppt["value"]).replace("{type}", ppt["type"]).replace("{name}", ppt["name"])
    return master_text.replace("{name}", name).replace("{pairs}", pairs)

def analyze_master() -> str:
    m = re.findall(master_ptn, original_str)
    txt = ""
    for one_class in m:
        m_name = re.search(master_name_ptn, one_class)
        if m_name:
            name = m_name.group("name")
            finded_all = re.findall(master_properties_ptn, one_class)
            ppts = []
            for ppt in finded_all:
                ...
                m_repeated = re.search(r"RepeatedField<(?P<t_name>\w+)>", type_)
                if m_repeated:
                    type_ = f"List<{m_repeated.group('t_name')}>"
                ppts.append({"field_name": field_name, "value": value, "type": type_, "name": name_})
            txt += gen_master(name, ppts)
    return txt
